...
if (!process.env.REDIS_URL) {
  ...
  const pollingJob = { id: "polling-fallback", name: "ledger-outbox-drain-poll" } as import("bullmq").Job;

  async function pollDrain() {
    try {
      await handleLedgerOutboxDrainJob(pollingJob);
    } catch (err) {
      console.error("[ledger-outbox-drain] polling invocation failed", err);
    }
  }

  // Run immediately on start, then every 60 s.
  void pollDrain();
  setInterval(() => { void pollDrain(); }, 60_000);
} else {
  startWorkers();
}
...
// Ledger event outbox drain — every 60 seconds. Drains PENDING outbox rows
// by calling emitLedgerEvent, ensuring no proof-trail event is permanently
// lost due to a transient emitter failure.
const ledgerOutboxWorker = new Worker(
  "ledger-outbox-drain",
  handleLedgerOutboxDrainJob,
  redisConnection,
);
ledgerOutboxWorker.on("completed", (job, result) =>
  console.log("Ledger outbox drain completed", job.id, job.name, result),
);
ledgerOutboxWorker.on("failed", (job, err) =>
  console.error("Ledger outbox drain failed", job?.id, err.message),
);

ledgerOutboxQueue.add(
  "ledger-outbox-drain-60s",
  {},
  {
    repeat: { every: 60 * 1000 },
    jobId: "ledger-outbox-drain-60s-repeatable",
  },
).catch((err) => console.warn("[ledger-outbox-drain] Failed to register repeatable job:", err));
