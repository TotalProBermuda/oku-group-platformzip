...
const ROLE_LABEL: Record<string, string> = {
  SUPERADMIN: "Superadmin", ADMIN_COMMERCIAL: "Admin — Commercial", ADMIN_IR: "Admin — IR",
  ADMIN_HR: "Admin — HR", INFLUENCER: "Influencer", PARTNER: "Partner", INVESTOR: "Investor",
  STAFF_OKU: "Staff (OKÜ)", STAFF_CATCH: "Staff (CATCH)", RESTAURANT_HOST: "Restaurant Host",
  STREETSIDE_HOST: "Streetside Host", REFERRER: "Referrer", ATTENDEE: "Attendee", VISITOR: "Visitor",
};
...
export default function ProfileDrawer({ profile, onClose, onRefresh }: ProfileDrawerProps) {
  ...
  function roleToCategory(roleKeys: string[]): string {
    if (roleKeys.some(r => ["SUPERADMIN","ADMIN_COMMERCIAL","ADMIN_IR","ADMIN_HR"].includes(r))) return "admin";
...
const ALL_ROLES: { key: string; label: string; group: string }[] = [
  ...
  { key: "ADMIN_COMMERCIAL", label: "Admin — Commercial",  group: "Admin" },
