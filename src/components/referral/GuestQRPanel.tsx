...
export interface GuestQRPanelProps {
  ...
  referralCode: string;
  /**
   * Optional ISO date string (e.g. `"2026-08-01"`) to pre-fill the date picker
   * in the reservation wizard. When provided, appended as `&date=<value>` after
   * the `ref` param. The guest can still change the date in the wizard.
   * Omit on general-purpose referral QRs (taxi, concierge, partner, influencer).
   */
  date?: string;
  /**
...
export function GuestQRPanel({
  ...
  useEffect(() => {
    ...
    function buildUrl(path: string): string {
      // Use URL + URLSearchParams so query separators (? vs &) are always
      // correct regardless of whether appendRefQuery is true or false and
      // whether `path` already contains a query string.
      const url = new URL(path, origin);
      if (appendRefQuery) url.searchParams.set("ref", referralCode);
      if (date) url.searchParams.set("date", date);
      return url.toString();
    }

    setGuestUrl(buildUrl(qrPath));
    setManualUrl(buildUrl(fallbackPath));
  }, [locale, referralCode, date, destinationPath, manualEntryPath, appendRefQuery]);
