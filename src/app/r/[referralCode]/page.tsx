...
export default function ReferralLandingPage() {
  ...
  const [bookingPendingApproval, setBookingPendingApproval] = useState(false);
  ...
  async function handleBookingSubmit(data: GuestBookingFormData) {
    ...
    try {
      ...
      // Read the body BEFORE mutating any state — the pendingApproval flag
      // determines which success screen to show.
      const body = await res.json().catch(() => ({}));
      if (res.ok) {
        const isPending = body.pendingApproval === true;
        setBookingPendingApproval(isPending);
        setBookedOk(true);
        return { ok: true, pendingApproval: isPending };
      }
      // Translate known API error codes; fall back to the raw message.
            ...
            {bookedOk ? (
              <div style={{ padding: "60px 24px", textAlign: "center" }}>
                {bookingPendingApproval ? (
                  ...
                  <>
                    <div style={{ width: 80, height: 80, borderRadius: "50%", background: "rgba(245,158,11,0.2)", border: "2px solid rgba(245,158,11,0.5)", display: "inline-flex", alignItems: "center", justifyContent: "center", marginBottom: 24 }}>
                      <span style={{ fontSize: 36, color: "#f59e0b" }}>⏳</span>
                    </div>
                    <div style={{ fontFamily: "var(--font-heading)", fontSize: 28, color: "#fff", letterSpacing: "-0.02em", marginBottom: 10 }}>
                      {tt("pendingApprovalTitle", "Request Received")}
                    </div>
                    <div style={{ fontSize: 14, color: "rgba(255,255,255,0.5)", marginBottom: 6 }}>
                      {tt("pendingApprovalBody", "This space requires host approval. We'll confirm your reservation shortly.")}
                    </div>
                  </>
                ) : (
                  ...
                  <>
                    ...
                    <div style={{ fontFamily: "var(--font-heading)", fontSize: 28, color: "#fff", letterSpacing: "-0.02em", marginBottom: 10 }}>
                      {tt("successTitle", "You're on the list")}
                    </div>
                    <div style={{ fontSize: 14, color: "rgba(255,255,255,0.5)", marginBottom: 6 }}>
                ...
                <button onClick={() => { setBookedOk(false); setBookingPendingApproval(false); setActiveTab("menu"); }} style={{
