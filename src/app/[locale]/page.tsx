...
export default async function LocaleHomePage({ params }: { params: Promise<{ locale: string }> }) {
  ...
  const isHost       = roles.some((r) => ["RESTAURANT_HOST", "STREETSIDE_HOST"].includes(r));
  ...
  function getRoleLabel(): string {
    if (isAdmin)          return common.roleAdmin || "Administrator";
    if (isStreetsideHost)  return common.roleStreetsideHost || "Streetside Host";
    if (isHost)           return common.roleRestaurantHost || "Restaurant Host";
    if (isInfluencer)     return common.roleInfluencer || "Influencer";
    if (isReferrer)       return common.roleReferrer || "Referrer";
    if (isPartner)        return common.rolePartner || "Partner";
    if (isInvestor)       return common.roleInvestor || "Investor";
    if (isStaff)          return common.roleStaff || "Staff";
    return common.roleMember || "OKÜ Member";
  }
  ...
  const hostQuickActions = [
    { href: "/host/streetside-team", icon: "🚶", label: common.quickHostStreetside || "Streetside Hosts",   desc: common.quickHostStreetsideDesc || "Who's on shift right now" },
    { href: "/host/dashboard",       icon: "📋", label: common.quickHostReservations || "Reservations",      desc: common.quickHostReservationsDesc || "Queue · referrals · walk-ins" },
    { href: "/host/operations",      icon: "🏢", label: common.quickHostFloor || "Floor View",               desc: common.quickHostFloorDesc || "Full floor · zone capacity" },
    { href: "/host/dashboard",       icon: "⚠️", label: common.quickHostIncidents || "Incidents",            desc: common.quickHostIncidentsDesc || "Losses & no-shows today" },
  ];

  const memberQuickActions = [
    { href: "/my/membership", icon: "◇", label: common.quickMyMembership || "My Membership", desc: common.quickMyMembershipDesc || "Status & benefits" },
    { href: "/my/tickets",    icon: "🎟", label: common.quickMyTickets || "My Tickets",       desc: common.quickMyTicketsDesc || "Upcoming events" },
    { href: "/my/orders",     icon: "📦", label: common.quickMyOrders || "My Orders",         desc: common.quickMyOrdersDesc || "Purchase history" },
    { href: localePath(safeLocale, "/series"), icon: "✦", label: common.quickBrowseEvents || "Browse Events", desc: common.quickBrowseEventsDesc || "Curated experiences" },
  ];

  const referrerQuickActions = [
    { href: "/referrer/dashboard", icon: "🔗", label: common.quickReferrerLink || "My Referral Link",   desc: common.quickReferrerLinkDesc || "Copy & share with guests" },
    { href: "/referrer/dashboard", icon: "👥", label: common.quickReferrerPipeline || "Guest Pipeline", desc: common.quickReferrerPipelineDesc || "Track referrals & conversions" },
    { href: localePath(safeLocale, "/series"), icon: "✦", label: common.quickBrowseEvents || "Browse Events", desc: common.quickBrowseEventsDesc || "Curated experiences" },
  ];

  return (
    <div>
      {/* ── HERO: visitor vs member view ─────────────────────────────── */}
      {session?.user ? (
        /* ── SIGNED-IN MEMBER DASHBOARD HERO ────────────────────────── */
        <MemberHero
          firstName={firstName}
          roleLabel={roleLabel}
          dashHref={dashHref}
          welcomeBack={common.welcomeBack || "Welcome back,"}
          goToDashboard={common.goToDashboard || "Go to Dashboard →"}
          quickActions={
            (isHost && !isStreetsideHost) ? hostQuickActions
            : (isAdmin || isStaff || isStreetsideHost) ? []
            : isReferrer ? referrerQuickActions
            : memberQuickActions
          }
        />
      ) : (
        /* ── VISITOR MARKETING HERO ──────────────────────────────────── */
