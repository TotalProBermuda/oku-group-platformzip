...
async function main() {
  ...
  const commercial   = await mkUser("commercial@oku.local", "Carlos Mendez",      "FB_DIRECTOR");
  ...
  const host1User    = await mkUser("host1@oku.local",      "Rafael Núñez",       "RESTAURANT_SUPERVISOR");
  ...
  const host1Profile = await prisma.restaurantHostProfile.upsert({
    where: { userId: host1User.id },
    update: { venueId: goldHouse.id, displayName: "Rafael N.", isActive: true },
    create: { userId: host1User.id, venueId: goldHouse.id, displayName: "Rafael N.", isActive: true },
  });
  const host2Profile = await prisma.restaurantHostProfile.upsert({
    where: { userId: host2User.id },
    update: { venueId: goldHouse.id, displayName: "Camila S.", isActive: true },
    create: { userId: host2User.id, venueId: goldHouse.id, displayName: "Camila S.", isActive: true },
  });
  ...
  console.log("  F&B Director:          commercial@oku.local     (Carlos Mendez)");
  ...
  console.log("  Restaurant Supervisor: host1@oku.local          (Rafael Núñez)");
...
async function ensureCommissionRuleDefaults() {
  // Seed four GLOBAL commission rule defaults — one per CommissionTierType.
  ...
  for (const r of tiers) {
    const existing = await prisma.commissionRule.findFirst({
      where: { scopeType: "GLOBAL", tier: r.tier },
      select: { id: true },
    });
    ...
    console.log(`  [commission-rules] created GLOBAL/${r.tier} @ ${r.percentageBps} bps`);
  }
}

main()
  .then(() => ensureCybersourceActiveForPanama())
  .then(() => ensureCommissionRuleDefaults())
